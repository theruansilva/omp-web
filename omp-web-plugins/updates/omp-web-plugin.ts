...
import type { HtmlTemplateTag, OmpWebComponentStatus, OmpWebPlugin, OmpWebStatusResponse, PluginRuntimeState, WorkspacePanelTerminal } from "@ProgmRuanSilva/omp-web/plugin-api";
...
function renderComponent(html: HtmlTemplateTag, component: OmpWebComponentStatus): TemplateResult {
...
function updatesRuntimeHintFromModuleUrl(moduleUrl: string): UpdatesRuntimeHint {
  try {
    const dockerMode = new URL(moduleUrl).searchParams.get("ompWebDockerMode");
...
function renderCommands(html: HtmlTemplateTag, terminal: WorkspacePanelTerminal | undefined, status: OmpWebStatusResponse): TemplateResult | undefined {
