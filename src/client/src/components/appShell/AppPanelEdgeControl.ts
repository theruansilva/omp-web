...
export class AppPanelEdgeControl extends LitElement {
  ...
  static override styles = css`
    ...
    .resize-handle::after { content: ""; position: absolute; top: 0; bottom: 0; left: 50%; width: 1px; transform: translateX(-50%) scaleX(1); transform-origin: center; background: transparent; transition: transform .12s ease, background .12s ease, opacity .12s ease; }
    .resize-handle:hover::after, .resize-handle:focus-visible::after, :host([resizing]) .resize-handle::after { transform: translateX(-50%) scaleX(3); background: var(--pi-accent); opacity: .72; }
