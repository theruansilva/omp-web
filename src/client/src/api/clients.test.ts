...
describe("machine-scoped runtime API", () => {
  it("reads machine OMP status through the gateway route", async () => {
    const fetchMock = stubJsonFetch({
      packageName: "@ProgmRuanSilva/omp-web",
      generatedAt: "now",
      components: {
        web: { component: "web", label: "OMP", available: true, stale: false },
        sessiond: { component: "sessiond", label: "OMP Session Daemon", available: true, stale: false },
      },
