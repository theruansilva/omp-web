...
type SchedulePromptParamsType = Static<typeof SchedulePromptParams>;
...
export function createSchedulePromptToolDefinition(
  ...
  const def = {
    ...
    async execute(
      ...
      const p = params as SchedulePromptParamsType;
        ...
        switch (action) {
          case "add": {
            if (!p.schedule || !p.prompt) {
              ...
              if (!p.schedule) missing.push("'schedule'");
            ...
            const jobName = p.name || `job-${nanoid(6)}`;
            ...
            const type = (p.type || "cron") as CronJobType;
            ...
            return {
              ...
              details,
            } as AgentToolResult<SchedulePromptToolDetails>;
          }

          case "remove": {
            if (!p.jobId) throw new Error("jobId is required for remove action");
          ...
          case "enable":
          case "disable": {
            if (!p.jobId) throw new Error(`jobId is required for ${action} action`);
          ...
          case "update": {
            if (!p.jobId) throw new Error("jobId is required for update action");
