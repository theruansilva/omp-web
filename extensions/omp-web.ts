...
const serviceNames = ["omp-web-sessiond.service", "omp-web.service", "omp-web-ui-dev.service"];
const macLogPaths = ["sessiond.log", "web.log", "ui-dev.log"].map((name) => join(homedir(), ".omp-web", "logs", name));
...
async function runOmpWeb(args: string[], env: NodeJS.ProcessEnv = {}): Promise<{ code: number; output: string }> {
  if (existsSync(cliPath)) {
    return run(process.execPath, [cliPath, ...args], env);
  }
  return run("omp-web", args, env);
}
...
async function boundedLogs(): Promise<{ code: number; output: string }> {
  if (process.platform === "darwin") {
    ...
    if (existingLogs.length === 0) return { code: 1, output: "No PI WEB log files found in ~/.omp-web/logs." };
...
export default function ompWebExtension(pi: ExtensionAPI): void {
  pi.registerCommand("omp-web", {
    ...
    async handler(args, ctx) {
      ...
      if (subcommand === "help") {
        ctx.ui.notify(`PI WEB commands:\n\n${subcommands.map((command) => `/omp-web ${command}`).join("\n")}\n\nLogs are bounded to the last 100 service log lines in the Pi command. Use \`omp-web logs\` in a shell to follow logs.`, "info");
        return;
      }
      ...
      if (!isSubcommand(subcommand)) {
        ctx.ui.notify(`Unknown omp-web command: ${subcommand}. Try /omp-web help.`, "error");
        return;
      }

      if (subcommand === "stop" || subcommand === "uninstall") {
        const ok = await ctx.ui.confirm(`omp-web ${subcommand}`, `Run omp-web ${subcommand}?`);
        if (!ok) return;
      }

      if (subcommand === "logs") {
        showResult(ctx, "omp-web logs", await boundedLogs());
        return;
      }

      showResult(ctx, `omp-web ${subcommand}`, await runOmpWeb([subcommand, ...rest]));
    },
  });
}
