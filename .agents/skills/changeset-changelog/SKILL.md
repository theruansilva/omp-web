...
This includes repo-only material such as agent skills under `.agents/`, internal docs, CI config, and developer tooling. If you are unsure whether a path ships, check it against `package.json` `files` (or run `bun run pack:dry`); when a change lives entirely outside the published files, skip the changeset unless the user explicitly wants it recorded.
